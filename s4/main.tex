...
\begin{frame}{Take Home Assignment 1: Common issues}
  \begin{itemize}
    \item[(3a)] Backwards Induction (BI)
  \end{itemize}
  \begin{figure}[!h]
    \begin{center}
...
\begin{frame}{Take Home Assignment 1: Common issues}
  ...
  \begin{align*}
    (preferred\ strategies\ for\ player\ 1,\ preferred\ strategies\ for\ player\ 2)=(Rrr^{'},R^{'}L^{''})
  \end{align*}
\end{frame}
...
\begin{frame}{PS4, Ex. 7: To keep or split (backwards induction)}
  ...
  \nth{1} stage: Player 1 chooses Keep or Split. Player 2 observes the choice.\\\medskip
  \nth{2} stage: They play the static game and payoffs are realized (doesn't matter who "moves first" as one's actions are private information till the other has moved).
  \begin{figure}[!h]
    \begin{center}
    \def\svgwidth{\columnwidth}
