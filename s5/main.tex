...
% Select what to do with command \intuition:
  % \newcommand{\intuition}[1]{\par {[...] \par}}  % intuition not shown
  \newcommand{\intuition}[1]{\par {#1 \par}} % intuition shown
...
\begin{frame}{PS5, Ex. 7.b: Dynamic game (proper subgames)}
  \begin{multicols}{2}
    ...
    \begin{align*}
      SPNE=(s_1^{*},s_2^{*})=(R\ R'',\ L')
    \end{align*}
    Which coincides with the backwards induction solution from Ex. 1!
    \vfill\null \columnbreak
    \begin{figure}[!h]
      \center
      \def\svgwidth{\columnwidth}
